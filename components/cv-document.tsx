...
// `embedded` renders inside the site shell (/about): no back link, no second <main>, toolbar not sticky
// so it can't collide with the fixed site header. Standalone (/cv) is the chrome-free shareable version.
export default function CVDocument({ embedded = false }: { embedded?: boolean }) {
  ...
  return (
    <div className="min-h-screen bg-background text-foreground print:bg-white print:text-slate-900">
      <nav
        ...
        className={
          embedded
            ? "no-print border-b border-foreground/10"
            : "no-print sticky top-0 z-50 border-b border-foreground/10 bg-background/85 backdrop-blur-xl"
        }
      >
        <div
          className={`mx-auto flex max-w-4xl flex-wrap items-center gap-3 px-4 py-3 sm:px-6 ${embedded ? "justify-end" : "justify-between"}`}
        >
          {!embedded && (
